...
class ANETdetection(object):
    ...
    def _import_ground_truth(self, annotation_path):
        gtsegments = np.load(os.path.join(annotation_path, "segments.npy"), allow_pickle=True)
        gtlabels = np.load(os.path.join(annotation_path, "labels.npy"), allow_pickle=True)
        videoname = np.load(os.path.join(annotation_path, "videoname.npy"), allow_pickle=True)
        # videoname = np.array([i.decode("utf8") for i in videoname])
        subset = np.load(os.path.join(annotation_path, "subset.npy"), allow_pickle=True)
        # subset = np.array([s.decode("utf-8") for s in subset])
        classlist = np.load(os.path.join(annotation_path, "classlist.npy"), allow_pickle=True)
        # classlist = np.array([c.decode("utf-8") for c in classlist])
        duration = np.load(os.path.join(annotation_path, "duration.npy"), allow_pickle=True)
        ambilist = os.path.join(annotation_path, "Ambiguous_test.txt")
        # import pdb;pdb.set_trace()
        try:
