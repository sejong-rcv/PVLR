...
from model.prob_encoder import TextEncoder
...
class TSM(torch.nn.Module):
    def __init__(self, actiondict, actiontoken, inp_actionlist,**args):
        ...
        """
        Linear Projection for CLIP visual feature
        """
        self.fc_clip = nn.Linear(1024,1024)
        ...
        Prob. embedding
        """
        self.snippet_prob_encoder = SnippetEncoder(2048, prob=True)
        """
    ...
    def forward(self, inputs, clip_feat, itr, split, **args):
        ...
        clip_feat = self.fc_clip(clip_feat)
        b,c,n=feat.size()
        ...
        return {'feat':nfeat_out0.transpose(-1, -2),'text_feat':text_feature,'feat_final':nfeat_out.transpose(-1,-2), 'cas':cas.transpose(-1,-2), 'attn':x_atn.transpose(-1,-2), \
                ...
                'n_ffeat':n_ffeat.transpose(-1,-2),'o_ffeat':o_ffeat.transpose(-1,-2), 'mu_v':mu_v, 'var_v':var_v, 'emb_v':emb_v
                }